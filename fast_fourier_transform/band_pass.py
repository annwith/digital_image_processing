...
def update_radius(smaller_radius = 10, bigger_radius = 20):
    """
    Update the band-pass filter and the band-pass filtered image.

    Args:
        radius: Radius of the band-pass filter.
    """
    ...
    # Apply the band-pass filter to the magnitude spectrum
    band_pass_filter = magnitude_spectrum.copy()
    band_pass_filter[~band_pass_mask] = 0 # Set False values to 0

    # Apply the band-pass filter to the image
    image_band_pass = fshift * band_pass_mask

    # Apply the inverse Fourier transform
    image_band_pass = np.fft.ifft2(np.fft.ifftshift(image_band_pass)).real
    ...
    # Displays the band-pass filtered image
    axs[0, 1].imshow(image_band_pass, cmap='gray')
    ...
    # Displays the filter
    axs[1, 1].imshow(band_pass_filter, cmap='gray')
    axs[1, 1].set_title('Band-pass Filter')
    axs[1, 1].axis('off')
...
parser = argparse.ArgumentParser(
    description='Load a image and apply a band-pass filter.')
